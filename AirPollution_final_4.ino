...
void setup() {
  ...
  if (wifi_connected) {
    ...
    Serial.println("1.UTC :-: Time :-: Date: " + time_stamp); // For UTC, take timeoffset as 0
    Serial.println("");
    previous_ts=ts;/*anu*/
...
void loop() {
  ...
  debug_info("\ncal_status_avg5:");
  debug_info(flag);
  if(previous_ts!=0 || nw_is_connected()){
    Serial.println("nw_is_connected status");
    Serial.println(wifi_connected,DEC); // For UTC, take timeoffset as 0
    Serial.println("");
    read_data_from_hw();
  ...
  debug_info(wifi_connected=nw_is_connected());
...
void read_data_from_hw() {
  ...
  if (millis() - last_ms_read_data_15 > TIME_INTERVAL_15_SEC) {
    Serial.print("lm1:");
    Serial.println(last_ms_read_data_15,DEC); // For UTC, take timeoffset as 0
    ...
    if (nw_is_connected()){
      ...
      timestamp_s = get_date_timestamp_str(time_stamp, 0);
      Serial.println(time_stamp,DEC); // For UTC, take timeoffset as 0
      Serial.println("");
    }else{
       time_stamp = previous_ts+TIME_INTERVAL_15_SEC;
       Serial.println(time_stamp,DEC); // For UTC, take timeoffset as 0
       Serial.println("");
    }
    ...
    */
    ...
    Serial.println("2.UTC :-: Time :-: Date: " + timestamp_s); // For UTC, take timeoffset as 0
    Serial.println("");
